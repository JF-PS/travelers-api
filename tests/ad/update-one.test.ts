...
describe("AdService ==> updateOne Tests : ", () => {
  /**
   *  Test 1 :
   *  title: "User does not exist"
   *  info: « Can't update ad if user doesn't exist »
   */
  test("User does not exist", async () => {
    ...
    // Act
    const response: any = await service.updateOne(1, ad, 1);
   ...
   *  info: « Can't update ad if user not validate his account »
   */
   ...
   *  info: « We can not modify the ad of
  ...
  test("Update Ad OK", async () => {
    ...
    const updateOne = (mockAdRepository.updateOne = jest.fn());
    updateOne.mockReturnValue(expected);
